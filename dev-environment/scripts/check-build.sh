...
##############################################################################
# 빌드 검사 스크립트 (TypeScript Compilation)
# Docker Compose 환경에서 모든 서비스 및 라이브러리의 빌드를 검사합니다.
##############################################################################
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}🔨 빌드 검사 시작 (TypeScript)${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
if ! docker ps --format '{{.Names}}' | grep -q "^${CONTAINER_NAME}$"; then
    ...
    echo ""
    echo -e "${BLUE}로컬에서 직접 빌드 검사를 실행합니다...${NC}"
    echo ""
    
    cd "$PROJECT_ROOT"
    
    # 전체 프로젝트에 대해 빌드 검사 실행
    echo -e "${BLUE}🔧 전체 프로젝트 빌드 중...${NC}"
    echo -e "${CYAN}이 작업은 수 분이 걸릴 수 있습니다...${NC}"
    echo ""
    
    if pnpm nx run-many -t build --all; then
        echo ""
        echo -e "${GREEN}✅ 빌드 검사 성공!${NC}"
        echo -e "${GREEN}========================================${NC}"
        exit 0
    else
        echo ""
    ...
    # Docker 컨테이너 내에서 빌드 검사 실행
    echo -e "${BLUE}🔧 Docker 컨테이너 내에서 빌드 검사 중...${NC}"
    echo -e "${CYAN}이 작업은 수 분이 걸릴 수 있습니다...${NC}"
    echo ""
    ...
    # Docker 컨테이너 내에서 빌드 실행
    if docker exec "$RUNNING_CONTAINER" pnpm nx run-many -t build --all; then
        echo ""
        echo -e "${GREEN}✅ 빌드 검사 성공!${NC}"
        echo -e "${GREEN}모든 서비스와 라이브러리가 정상적으로 빌드되었습니다.${NC}"
        echo -e "${GREEN}========================================${NC}"
        exit 0
    else
        echo ""
