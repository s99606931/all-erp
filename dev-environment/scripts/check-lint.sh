...
##############################################################################
# 문법 검사 스크립트 (ESLint)
# Docker Compose 환경에서 모든 서비스 및 라이브러리의 문법을 검사합니다.
##############################################################################
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}📝 문법 검사 시작 (ESLint)${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
if ! docker ps --format '{{.Names}}' | grep -q "^${CONTAINER_NAME}$"; then
    ...
    echo ""
    echo -e "${BLUE}로컬에서 직접 문법 검사를 실행합니다...${NC}"
    echo ""
    
    cd "$PROJECT_ROOT"
    
    # 전체 프로젝트에 대해 문법 검사 실행
    echo -e "${BLUE}🔍 전체 프로젝트 문법 검사 중...${NC}"
    
    if pnpm nx run-many -t lint --all; then
        echo ""
        echo -e "${GREEN}✅ 문법 검사 성공!${NC}"
        echo -e "${GREEN}========================================${NC}"
        exit 0
    else
        echo ""
    ...
    # Docker 컨테이너 내에서 문법 검사 실행
    echo -e "${BLUE}🔍 Docker 컨테이너 내에서 문법 검사 중...${NC}"
    ...
    # Docker 컨테이너 내에서 문법 검사 실행
    if docker exec "$RUNNING_CONTAINER" pnpm nx run-many -t lint --all; then
        echo ""
        echo -e "${GREEN}✅ 문법 검사 성공!${NC}"
        echo -e "${GREEN}========================================${NC}"
        exit 0
