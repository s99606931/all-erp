...
1.  `ai-service` **FastAPI 프로젝트 스캐폴딩** (Poetry 사용 권장)
2.  **Milvus 연동 및 Collection(Index) 설계** (pymilvus 활용)
3.  **RAG 파이프라인 구현**:
    - LangChain Python 사용
    - 문서 로더 (PyPDF, TextLoader) 및 텍스트 분할기 구현
    - 임베딩 모델 연동 (OpenAI Embeddings)
4.  **API 구현**:
    - `/api/v1/chat`: RAG 기반 질의응답
    - `/api/v1/documents`: 문서 업로드 및 임베딩 처리
5.  자동 분개 및 이상 탐지 로직 구현 (Pandas, Scikit-learn 활용)

## 5. 승인 기준 (Acceptance Criteria)
- [ ] 사용자의 자연어 질문에 대해 RAG를 활용하여 정확한 답변을 제공해야 한다.
- [ ] 업로드된 문서가 Vector DB에 정상적으로 임베딩되어 저장되어야 한다.
- [ ] 영수증 이미지를 업로드하면 OCR을 통해 텍스트가 추출되어야 한다.
