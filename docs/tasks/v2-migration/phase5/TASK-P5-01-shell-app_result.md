...
- [x] **로그인 페이지** 구현
- [x] **404 Not Found 페이지** 컴포넌트 분리
- [x] **ProtectedRoute** 컴포넌트 추가 (인증 보호)
- [x] **ESLint 설정** 추가
- [x] **Vite 환경 변수 타입 정의** 추가
...
│   ├── Router.tsx            # 라우팅 + Remote 앱 동적 로드 + 인증 보호
│   └── Layout.tsx            # 메인 레이아웃
├── components/
...
│   └── ProtectedRoute.tsx    # 🆕 보호된 라우트 (인증 체크)
├── pages/                    # 🆕 페이지 컴포넌트
│   ├── Login.tsx             # 🆕 로그인 페이지
