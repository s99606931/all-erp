...
1.  **폴더 구조 준수**: `docs/guides/project-structure.md`에 정의된 폴더 구조를 엄격히 따릅니다.
2.  **공통 부트스트랩 사용**: 모든 서비스의 `main.ts` 작성 시 `bootstrapService`를 사용합니다.
3.  **안전성 우선**: 모든 비동기 로직에는 `try-catch`를 적용하고, 적절한 에러 로그를 남깁니다.
